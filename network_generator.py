...
def main():
    ...
    parser.add_argument('-model', action='store',dest='model', required=False, type=str)
    parser.add_argument('-path_network', action='store',dest='path_network', required=False, type=str)
    parser.add_argument('-p_ER', action='store',dest='p_ER', required=False, type=float, default=0)
    parser.add_argument('-rp_size','--rp_size', nargs='+', help='Size of random partition graph groups', required=False, type=int, default=[0])
    parser.add_argument('-p_within','--p_within', nargs='+', help='probability of edges for different groups (descending order), take 2 elements rn', required=False, type=float, default=[0])
