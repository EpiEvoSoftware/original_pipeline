...
def _percentile_to_index(percentile, node_per_percent):
    ...
    if len(percentile) != 2:
        raise CustomizedError(
            f"The percentile range {percentile} is not a list of two element.")
    ...
    if not_int or not_0_100:
        raise CustomizedError(
            f"The percentile range {percentile} is not a list of element of two 0 - 100 integers.")
    if h_per <= l_per:
        raise CustomizedError(
            f"The percentile range {percentile} is not a valid interval.")
...
def read_network(network_path):
    ...
    if not network_path.exists():
        raise FileNotFoundError(
            f"The provided networkX path '{network_path}' doesn't exist. Please run network_generation first before running this script and make sure the given working directory is correct.")
...
def match_ranking(nodes_sorted, taken_hosts, rank):
    ...
    if rank > ntwk_size:
        raise CustomizedError(
            f"Your provided ranking {rank} exceed host size {ntwk_size}.")
    host = nodes_sorted[rank - 1]
    if host in taken_hosts:
        raise CustomizedError(
            f"Host of specified rank {rank} is already taken.")
    return host
...
def match_percentile(nodes_sorted, taken_hosts, percentile):
    ...
    if high_idx > low_idx:
        ...
        if available_host == []:
            raise CustomizedError(
                f"There is no host left to match in the percentile {percentile}.")
        host = sample(available_host, 1)[0]
        return host
    raise CustomizedError(
        f"There is no host left to match in the range {percentile}%.")
...
def write_match(match_dict, wk_dir):
    ...
    _save_dict_to_csv(sorted_match, os.path.join(
        wk_dir, "seed_host_match.csv"))
...
def match_all_hosts(ntwk_, match_method, param, num_seed):
    ...
    if num_seed != len(param.keys()):
        raise CustomizedError(
            f"Please provide a matching parameter for each seed. (Length of matching parameter {len(param.keys())} doesn't match number of seed {num_seed})")
    if num_seed != len(match_method.keys()):
        raise CustomizedError(
            f"Please provide a matching method for each seed. (Length of matching method {len(match_method.keys())} doesn't match number of seed {num_seed})")
    if num_seed > ntwk_size:
        raise CustomizedError(
            f"It is not allowed to match {num_seed} seeds to {ntwk_size} hosts. Please reduce the number of seeds or increase the host population size.")
...
# Gather the nodes by their matching method
    ...
    for _, (seed_id, method) in enumerate(match_method.items()):
        ...
        # raise CustomizedError(
        #     f"Please provide a valid matching method in ('Ranking', 'Percentile', 'Random') instead of {method} for seed {seed_id}")
        if method in ['Ranking', 'Percentile', 'Random']:
    ...
    match_functions = {'Ranking': match_ranking,
                       'Percentile': match_percentile, 'Random': match_random}
    match_dict = {}
    for method in ['Ranking', 'Percentile', 'Random']:
        match_function = match_functions[method]
        for seed_id in dict_method_seeds[method]:
            matched_host = match_function(
                nodes_sorted, taken_hosts_id, param[seed_id])
...
def read_config_and_match(file_path, num_seed):
    ...
    run_seed_host_match(method=method, wkdir=config_all["BasicRunConfiguration"]["cwdir"], num_seed=num_seed,
                        path_matching=path_matching, match_scheme=match_method, match_scheme_param=match_method_param)
...
def run_seed_host_match(method, wkdir, num_seed, path_matching="", match_scheme="", match_scheme_param=""):
    ...
    try:
        if method == "user_input":
            if path_matching == "":
                raise CustomizedError(
                    "Path to the user-provided matching file (-path_matching) needs to be provided in user_provided mode.")
            elif os.path.exists(path_matching) == False:
                raise CustomizedError(
                    "Path to the user-provided matching file (-path_matching) doesn't exist.")
            else:
                read_user_matchingfile(path_matching)
        elif method == "randomly_generate":
            if match_scheme == "random":
                match_scheme = {
                    seed_id: "random" for seed_id in range(num_seed)}
                match_scheme_param = {
                    seed_id: None for seed_id in range(num_seed)}
            else:
                try:
                    match_scheme = json.loads(match_scheme)
                except json.decoder.JSONDecodeError:
                    raise CustomizedError(
                        f"The match_scheme is not a valid json format.")
            ...
            if list(set(match_scheme.values())) != ["random"]:
                try:
                    ...
                    raise CustomizedError(
                        f"The match_scheme_param is not a valid json format.")

            write_match(match_all_hosts(ntwk_=read_network(
                ntwk_path), match_method=match_scheme, param=match_scheme_param, num_seed=num_seed), wkdir)
            print(
                f"The matching process has successfully terminated. Please see working directory {wkdir}/seed_host_match.csv for the seed host matching file.")
        else:
            raise CustomizedError(
                f"Please provide a permitted method (-method): user_input/randomly_generate instead of your current input {method}.")
    except (CustomizedError, FileNotFoundError, ValueError) as e:
        print(
            f"Seed and host match - A violation of input parameters occured: {e}")
