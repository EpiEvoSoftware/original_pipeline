...
if [ -f "${causal_gene_path}" ]; then
    ...
    #    -gff ${gff} \
    #    -causal ${n_causal_genes} -es_low ${efsize_min} -es_high ${efsize_max} \
    #    -seeds_dir ${cwdir}/originalvcfs/
...
#Rscript  ${codes_dir}/transtree.r ${cwdir}
...
cat ${cwdir}/eference.snpsonly.fasta sample.SNPs_only.timed.fasta > ${cwdir}/sample.SNPs_only.timed.withref.fasta
