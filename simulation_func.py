...
def create_slimconfig(all_config):
	...
	with open(os.path.join(all_config["BasicRunConfiguration"]["cwdir"], "slim.params"), "w") as out_config:
		...
		out_config.write("seed_host_matching_path:" + os.path.join(slim_pars["cwdir"], "seed_host_match.csv") + "\n")
		...
		#	out_config.write("causal_gene_path:" + os.path.join(wk_dir, "/causal_gene_info.csv") + "\n")
...
def run_per_slim_simulation(slim_config_path, wk_dir, runid = 1):
	...
	subprocess.run(["slim", "-d", f"config_path=\"{slim_config_path}\"", "-d", f"runid={runid}", script_path])
	return(0)
...
def all_slim_simulation_by_config(all_config):
	...
	### And run post data processing for each folder.
	### No return value
	slim_pars = create_slimconfig(all_config)
	wk_dir = slim_pars["cwdir"]
	run_all_slim_simulation(slim_config_path = os.path.join(wk_dir, "slim.params"), slim_pars = slim_pars)
	

def run_all_slim_simulation(slim_config_path="", slim_pars={}):
	...
	def run_per_slim_simulation_helper(idx):
		run_per_slim_simulation(slim_config_path, slim_pars["cwdir"], idx)

	if run_check:
		create_slimscript(slim_pars)
		p = Pool()
		# p.starmap_async(run_per_slim_simulation, (slim_config_path, slim_pars["cwdir"], range(1, slim_pars["n_replicates"] + 1)))
		p.map(run_per_slim_simulation_helper, list(range(1, slim_pars["n_replicates"] + 1)))
		p.close()

		#if __name__ == '__main__':
		#	with Pool(processes=os.cpu_count() - 1) as pool:
		#		test = [pool.apply_async(run_per_slim_simulation, args=(slim_config_path, slim_pars["cwdir"], runid)) for runid in range(1, slim_pars["n_replicates"] + 1)]
		#		results = [ar.get() for ar in test]
		#	print(results)
...
all_config = read_params("/Users/px54/Documents/TB_software/V2_code/test/params_test.json")
slim_pars = create_slimconfig(all_config)
#create_slimscript(slim_pars)
#run_per_slim_simulation("/Users/px54/Documents/TB_software/V2_code/test/slim.params", "/Users/px54/Documents/TB_software/V2_code/test")

all_slim_simulation_by_config(all_config)



#for runid in range(1, slim_pars["n_replicates"] + 1):
#	print(type(runid))
#	output_path = os.path.join(slim_pars["cwdir"], str(runid))
#	if not os.path.exists(output_path):
#		os.makedirs(output_path)
#	else:
#		shutil.rmtree(output_path)           # Removes all the subdirectories!
#		os.makedirs(output_path)
#	script_path = os.path.join(slim_pars["cwdir"], "simulation.slim")
#	slim_config_path = os.path.join(slim_pars["cwdir"], "slim.params")
#	subprocess.run(["slim", "-d", f"config_path=\"{slim_config_path}\"", "-d", f"runid={runid}", script_path])
